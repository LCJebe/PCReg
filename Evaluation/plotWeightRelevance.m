...
W_forward = 0:10;
W_reverse = 10:-1:0;
ratio = W_forward ./ (W_forward + W_reverse);

precision = zeros(size(W1, 2), 4); % ratio x moment
num_goodInliers = zeros(size(W1, 2), 4); % ratio x moment
num_badInliers = zeros(size(W1, 2), 4); % ratio x moment

%%
tic
for moment = 1:4
    for i = 1:length(W_forward)

        weights.M1 = W_reverse(i); 
        weights.M2 = W_reverse(i);
        weights.M3 = W_reverse(i); 
        weights.M4 = W_reverse(i);

        % change weights for moment to look at to forward here
        if moment == 1
            weights.M1 = W_forward(i);
        elseif moment == 2
            weights.M2 = W_forward(i);
        elseif moment == 3
            weights.M3 = W_forward(i);
        elseif moment == 4
            weights.M4 = W_forward(i);
        end
        ...
        precision(i, moment) = inliersPrecision;
        num_goodInliers(i, moment) = inliers1;
        num_badInliers(i, moment) = inliers2;

    end % weight iteration
end % moment iteration
toc
%% Plots
close all;
...
figure()
plot(ratio, num_goodInliers);
title("Good Inliers");
legend('M1', 'M2', 'M3', 'M4');
grid;

figure()
plot(ratio, num_badInliers);
title("Bad Inliers");
legend('M1', 'M2', 'M3', 'M4');
grid;
